 ...
 * (Floor Management is this screen; Users/Sensors/Settings are
 * "SOON"-badged, matching the source design — not hidden, per the brief's
 * "disabled with a tooltip, never hidden silently" rule).
 */
  ...
  return (
    <aside className="flex w-[240px] flex-none flex-col overflow-y-auto border-r border-slate-200 bg-white">
      <nav className="flex flex-col gap-[2px] p-3">
        ...
        {ADMIN_NAV.administration.map((item) =>
          "active" in item && item.active ? (
            <div
              key={item.key}
              className="flex cursor-pointer items-center gap-[11px] rounded-lg bg-teal-100 px-[11px] py-[9px] text-[13.5px] font-semibold text-teal-700"
            >
              <Icon name={item.icon} size={18} />
              {item.label}
            </div>
