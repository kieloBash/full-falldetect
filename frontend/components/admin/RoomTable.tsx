    ...
    <div className="flex-1 overflow-y-auto px-6 py-5">
      <div className="mb-[14px] flex items-center justify-between">
        <div>
          ...
          <div className="mt-[2px] text-[12.5px] text-slate-600">{COPY.room.tableTitle}</div>
        </div>
        <button
          type="button"
          onClick={onAddRoom}
      ...
      <div className="overflow-x-auto rounded-[10px] border border-slate-200 bg-white">
        <div
          className={`grid min-w-[560px] ${ROOM_MGMT_GRID_COLS} border-b border-slate-200 bg-slate-50 px-[14px] py-[10px] text-[11px] font-semibold uppercase tracking-[.04em] text-slate-400`}
        >
          <div>Room</div>
          <div>Resident</div>
          <div>Sensor ID</div>
        ...
        {floor.rooms.length === 0 && <div className="px-4 py-10 text-center text-[13.5px] font-medium text-slate-400">{COPY.room.noRooms}</div>}
      </div>
    </div>
  );
}
